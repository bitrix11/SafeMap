...
- [x] **BUG (QA Antigravity) RESUELTO:** `_snap_to_grid` usaba la longitud en
      `math.cos()` al calcular la grilla de longitud. Corregido: se pasa la
      latitud del punto como parámetro `lat` a la función; `_obfuscate` lo
      suministra. El test `test_06_ofuscacion_coordenadas` actualizado valida
