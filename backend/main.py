...
def _snap_to_grid(coord: float, meters: float, is_lat: bool, lat: float = 0.0) -> float:
    """Snap coordenada a la grilla más cercana de `meters` metros.

    Para longitud, `lat` debe ser la latitud del punto: el tamaño angular de un
    metro en longitud depende del coseno de la LATITUD, no de la longitud.
    """
    if is_lat:
        deg_per_m = 1 / 111_320
    else:
        deg_per_m = 1 / (111_320 * math.cos(math.radians(lat)))
...
def _obfuscate(row: asyncpg.Record) -> ReportePublico:
    ...
    return ReportePublico(
        ...
        lng=_snap_to_grid(lng_raw, GRID_METERS, is_lat=False, lat=lat_raw),
